...
export default function PortfolioPieChart({ data }) {
  return (
    <div className="lg:col-span-1 bg-surface border border-border p-5 h-[380px] flex flex-col overflow-hidden">
      <h3 className="text-xs font-bold var(--text) uppercase tracking-widest mb-4 border-b border-border/50 pb-2">Asset Allocation</h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip 
              ...
              contentStyle={{ backgroundColor: 'var(--background)', border: '1px solid #333', color: 'var(--text)', fontSize: '10px' }} 
            />
          </PieChart>
        </ResponsiveContainer>
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-[9px] text-muted uppercase tracking-widest">Assets</span>
          <span className="text-xl font-bold var(--text) number-font">{data.length}</span>
        </div>
      </div>
