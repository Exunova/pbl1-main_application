    ...
    <a href={article.link || '#'} target="_blank" rel="noopener noreferrer"
      ...
      <div className="flex-1 flex flex-col justify-center min-w-0">
        <div className="flex items-center gap-2 mb-2">
          {(() => {
            const imgSrc = article.thumbnail?.url || article.favicon
            if (imgSrc) {
              return (
                <img src={imgSrc} alt="" className="w-4 h-4 rounded-full object-cover shrink-0"
                  onError={e => { e.target.style.display = 'none' }} />
              )
            }
            return <Newspaper size={10} style={{ color: color }} className="shrink-0" />
          })()}
