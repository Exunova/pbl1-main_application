        ...
        <div className="flex-1 overflow-auto p-3">
          {tab === 'chart' ? (
            ...
            infoLoading ? (
              ...
              <div className="text-xs text-white/30">No company data available</div>
            )
          ) : tab === 'financials' ? (
            infoLoading ? (
              <div className="text-xs text-white/40">Loading...</div>
              ...
              <table className="w-full text-xs text-white/70">
                ...
                <tbody>
                  {financialsRows.map(([dateStr, yearData]) => (
                    <tr key={dateStr} className="border-b border-white/5">
                      ...
                      <td className="text-right py-1.5">{yearData['Diluted EPS'] != null ? yearData['Diluted EPS'].toFixed(2) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <div className="text-xs text-white/30">No financial data available</div>
            )
          ) : (
            infoLoading ? (
              <div className="text-xs text-white/40">Loading...</div>
              ...
              <div className="space-y-3 text-xs text-white/70">
                {aboutInfo.longBusinessSummary && (
                  <p className="text-white/60 leading-relaxed">{aboutInfo.longBusinessSummary}</p>
                )}
                <div className="space-y-1">
                  {aboutInfo.website && (
                    <div>Website: <a href={aboutInfo.website} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">{aboutInfo.website}</a></div>
                  )}
                  {aboutInfo.sector && <div>Sector: {aboutInfo.sector}</div>}
                  {aboutInfo.industry && <div>Industry: {aboutInfo.industry}</div>}
                  {aboutInfo.fullTimeEmployees != null && <div>Employees: {aboutInfo.fullTimeEmployees.toLocaleString()}</div>}
                </div>
              </div>
            ) : (
              <div className="text-xs text-white/30">No about data available</div>
            )
          )}
        </div>
      </div>
