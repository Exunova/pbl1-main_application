      ...
      {loading ? (
        Array.from({ length: 4 }).map((_, i) => (
          ...
          >
            ...
            <div className="h-2.5 bg-border rounded w-full mb-1" />
            <div className="h-2.5 bg-border rounded w-5/6 mb-2" />
        ...
        articles.map((art, i) => (
          <a
            ...
            className="group block bg-surface border border-border rounded-lg p-3 hover:bg-border/30 transition-all duration-200"
          >

            {/* Thumbnail */}
            {art.thumbnail?.url && (
              <div className="mb-2 rounded overflow-hidden h-20 bg-border">
                <img
            ...
            {/* Title */}
            <p className="text-xs font-medium text-text leading-snug group-hover:text-accent transition-colors line-clamp-3 mb-1.5">
              {art.title}
            </p>
...
export default function CompareView() {
  ...
  <div className="flex flex-col h-full p-4 gap-5 overflow-auto custom-scrollbar">
    ...
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          className="bg-surface border border-border rounded px-2 py-1.5 text-xs text-text outline-none focus:border-accent/50 transition-colors"
        >
      ...
      <label className="flex items-center gap-1.5 text-xs text-muted ml-auto cursor-pointer select-none hover:text-text transition-colors">
        <input
          type="checkbox"
          checked={normalize}
    ...
    {/* ── Price Chart ── */}
    <div className="bg-surface rounded-xl border border-border p-4 h-56 shrink-0">
      {loading ? (
    ...
    <div className="grid grid-cols-2 gap-4">
      {[idx1, idx2].map((region, i) => (
        <div key={`${region}-${i}`} className="bg-surface rounded-xl border border-border p-3">
          <h3 className="text-[11px] font-bold text-muted uppercase tracking-widest mb-2 flex items-center gap-1.5">
