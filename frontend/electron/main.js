...
function startPythonBackend() {
  ...
  let pythonExe, pythonArgs, backendDir;

  if (isDev) {
    // Development: use venv Python to run ipc_main.py script
    backendDir = join(__dirname, "../../../backend/src");
    ...
    // Production: use frozen executable from extraResources
    backendDir = join(process.resourcesPath, "backend/src");
    pythonExe = join(process.resourcesPath, "python-bin", "ipc_main.exe");
    pythonArgs = [];
  }
  ...
  pythonProcess.on("exit", (code, signal) => {
    if (code !== 0) {
      console.error(
        `[Python IPC] Process exited with error code ${code}, signal ${signal}`,
      );
    } else {
      console.log(
        `[Python IPC] Process exited with code ${code}, signal ${signal}`,
      );
    }
    pythonProcess = null;
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("portfolio-add", (_, pos) =>
    sendToPython("portfolio_add", pos),
  );
  ipcMain.handle("portfolio-edit", (_, id, fields) =>
    sendToPython("portfolio_edit", { id, ...fields }),
  );
  ipcMain.handle("portfolio-delete", (_, id) =>
    sendToPython("portfolio_delete", { id }),
  );
  ipcMain.handle("portfolio-pnl", () => sendToPython("portfolio_pnl", {}));
  ipcMain.handle("portfolio-export", () =>
    sendToPython("portfolio_export", {}),
  );
  ipcMain.handle("portfolio-import", (_, data) =>
    sendToPython("portfolio_import", data),
  );
  ipcMain.handle("check-forex-rate", (_, params) =>
    sendToPython("check_forex_rate", params),
  );

  // Data fetching handlers — route to Python backend commands
  ipcMain.handle("fetchOHLCV", (_, ticker) =>
    sendToPython("ohlcv", { ticker }),
  );
  ...
  ipcMain.handle("fetchCompany", (_, ticker) =>
    sendToPython("company", { ticker }),
  );
  ipcMain.handle("fetchCompanies", (_, tickers) =>
    sendToPython("companies", { tickers }),
  );
  ipcMain.handle("fetchIndex", (_, idx) => sendToPython("index", { idx }));
  ipcMain.handle("fetchIndices", () => sendToPython("indices", {}));
  ipcMain.handle("triggerScrape", (_, type) =>
    sendToPython("scrape", { type }),
  );
  ...
  ipcMain.handle("get_scraped_tickers", () =>
    sendToPython("get_scraped_tickers", {}),
  );
  ...
  // Kill Python process on app exit (any reason - normal quit, force quit, or crash)
  app.on("before-quit", () => {
    if (pythonProcess) {
      console.log("[Python IPC] Killing backend process on app quit");
      pythonProcess.kill();
      pythonProcess = null;
    }
  });
