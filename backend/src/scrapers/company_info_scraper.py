...
Company Info scraper that fetches data from yfinance for multiple markets.
Run: run(output_dir)
"""
...
def run(output_dir):
    ...
    for market, tickers in MARKETS.items():
        ...
        for ticker in tickers:
            ...
            # Check cache first
            fresh, result = is_cache_fresh(f"company:{ticker}", COMPANY_TTL_SECONDS)
            if fresh and result:
                print("OK (cache)")
                ...
                try:
                    ...
                    cache_set(f"company:{ticker}", result)
...
if __name__ == "__main__":
    # Default DATA_DIR when run directly
