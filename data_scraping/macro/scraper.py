...
def parse_impact(row):
    ...
    filled = 0
    for star in stars:
        svg = star.evaluate_handle("node => node.parentElement")
        if svg:
...
def scrape_calendar(from_date, to_date, countries=None):
...
def main():
    ...
    events = scrape_calendar(from_date, to_date, target_countries)
